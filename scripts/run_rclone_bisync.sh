#!/usr/bin/env bash
# This script runs `rclone bisync` to bidirectionally sync a local Dropbox folder
# (specified by the `--local-dropbox-dir` flag) and a remote Dropbox folder (specified
# by the `--remote-dropbox-dir` flag). The `--remote-dropbox-dir` folder should exist
# on the `rclone` remote specified by the `--remote-name` flag. Note that the script
...
function usage {
  echo 'This script runs `rclone bisync` to bidirectionally sync a local Dropbox'\
    'folder (specified by the `--local-dropbox-dir` flag) and a remote Dropbox folder'\
    '(specified by the `--remote-dropbox-dir` flag). The `--remote-dropbox-dir`'\
    'folder should exist on the `rclone` remote specified by the `--remote-name`'\
    'flag. Note that the script requires `rclone` to be installed and configured.'
  echo
...
if [ ! command -v rclone &> /dev/null ]; then
  echo 'The `rclone` command is required by this script but could not be found.'\
    'Please install and configure `rclone` before running this script.'
  exit 2
fi
